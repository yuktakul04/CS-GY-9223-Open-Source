...
def test_delete_message_sends_expected_request_and_returns_true() -> None:
    ...
    with _transport_api(handler) as (api, transport):
        result = api.delete_message(message_id="m/1")

    assert result is True
    request = transport.requests[0]
    assert request.method == "DELETE"
    assert request.url.raw_path == b"/chat/messages/m%2F1"
...
def test_delete_message_returns_false_when_service_reports_failure() -> None:
    ...
    with _transport_api(handler) as (api, _transport):
        assert api.delete_message(message_id="m-1") is False


def test_get_message_requests_expected_path_and_returns_dto() -> None:
    """get_message maps generated MessageModel payload to MessageDTO."""
    ...
    with _transport_api(handler) as (api, transport):
        result = api.get_message(message_id="opaque-1")

    assert isinstance(result, MessageDTO)
    assert result.id == "opaque-1"
    request = transport.requests[0]
    assert request.method == "GET"
    assert request.url.path == "/chat/messages/opaque-1"
