...
from chat_client_service.provider import is_telegram_provider
...
def get_telegram_assistant() -> TelegramAssistantOrchestrator | None:
    """Return the env-configured Telegram assistant when available."""
    if not is_telegram_provider():
        return None
    return build_default_orchestrator(get_chat_client())
...
def telegram_webhook(
    ...
    if not is_telegram_provider():
        raise HTTPException(
            ...
            detail=(
                "Telegram webhook is disabled when CHAT_CLIENT_PROVIDER is not "
                "telegram."
            ),
        )
